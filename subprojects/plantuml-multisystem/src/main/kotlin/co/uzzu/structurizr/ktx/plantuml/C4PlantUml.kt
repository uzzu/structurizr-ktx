...
internal sealed class C4ElementType(val value: String) {
    internal object Default : C4ElementType("")
    internal object Db : C4ElementType("Db")

    companion object {

        val propertyName = "c4:element:type"

        fun of(value: String?): C4ElementType =
            when (value) {
                "Db" -> Db
                else -> Default
            }
...
internal sealed class C4RelationshipMode(
    val value: String,
    val macroBase: String = "Rel_$value"
) {
    internal object Rel : C4RelationshipMode("Rel", "Rel")
    internal object Neighbor : C4RelationshipMode("Neighbor")
    internal object Back : C4RelationshipMode("Back")
    internal object BackNeighbor : C4RelationshipMode("Back_Neighbor")
    internal object Lay : C4RelationshipMode("Lay", "Lay")

    companion object {
        val propertyName = "c4:layout:mode"

        fun of(value: String?): C4RelationshipMode =
            when (value) {
                "Neighbor" -> Neighbor
                "Back" -> Back
...
internal sealed class C4Direction(
    val value: String,
    val macroSuffix: String = "_${value.substring(0, 1)}"
) {
    internal object Default : C4Direction("", "")
    internal object Up : C4Direction("Up")
    internal object Down : C4Direction("Down")
    internal object Right : C4Direction("Right")
    internal object Left : C4Direction("Left")

    companion object {
        val propertyName = "c4:layout:direction"

        fun of(value: String?): C4Direction =
            when (value) {
                "Up" -> Up
                "Down" -> Down
                "Right" -> Right
...
private class C4DynamicViewBody(
    ...
    override fun StringBuilder.relationshipView(relationshipView: RelationshipView) {
        ...
        relationshipView.asDynamicViewText(style, config.escapedLineSeparator)
    }
}
...
private fun SoftwareSystem.asC4Text(escapedLineSeparator: String): String {
    val type = C4ElementType.of(properties[C4ElementType.propertyName])
